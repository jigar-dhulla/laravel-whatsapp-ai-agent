...
- `Wacli` service — wraps the wacli binary: `doctor`, `chats`, `groups`, `send`, `syncOnceExitIfIdleForFiveSeconds`, and `waitUntilUnlocked`. All commands automatically include `--store <dir>` when `WA_WACLI_STORE` is set.
- `WacliException` — thrown by `waitUntilUnlocked()` when the sync lock cannot be acquired within the configured number of attempts.
